...
public class Cell {
    ...
    public Cell checkWalls(Cell[][] celllist) {
        ...
        if (top != null && !top.visited && celllist[row][col].getWalls()[0] && top.getWalls()[2]) {
            neighbourspath.add(top);
        }

        if (right != null && !right.visited && celllist[row][col].getWalls()[1] && right.getWalls()[3]) {
            neighbourspath.add(right);
        }

        if (bottom != null && !bottom.visited && celllist[row][col].getWalls()[2] && bottom.getWalls()[0]) {
            neighbourspath.add(bottom);
        }

        if (left != null && !left.visited && celllist[row][col].getWalls()[3] && left.getWalls()[1]) {
